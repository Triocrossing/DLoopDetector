...
#include "../../demo/VocabularyBinary.hpp"
...
void TemplatedVocabulary<TDescriptor,F>::loadBin(const std::string &filename) {
  ...
  {
    std::cout<<"nodes: "<<i<<"/"<<voc.nNodes<<std::endl;
    ...
    auto db = boost::dynamic_bitset<>(voc.nodes[i].descriptor, voc.nodes[i].descriptor + 4);

    // ugly conversion but works
    std::string str;
    boost::to_string(db, str);
    m_nodes[nid].descriptor = std::bitset<256>(str);
  ...
  {
    std::cout<<"words: "<<i<<"/"<<voc.nWords<<std::endl;
